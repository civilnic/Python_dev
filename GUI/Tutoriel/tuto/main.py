...
class FileDictListModel(QAbstractListModel):
    ...
    def setData(self, index, value, role):
        ...
        if self._roles is None:
            self._roles = dict(zip(range(Qt.UserRole + 1, Qt.UserRole + 1 + len(decoded_value)),
                                   [v.encode('utf-8') for v in decoded_value.keys()]))
...
initialBooks = [{'title': "Commentarii de Bello Gallico", 'author': "Caius Iulius Caesar", 'genre': "Historic",
                 'publisher': "Les Belles Lettres", 'year': "1926",
                 'summary': "Julius Caesar\'s firsthand account of the Gallic Wars", 'price': "9.00"},
                {'title': "Προμηθεὺς δεσμώτης", 'author': "Αἰσχύλος", 'genre': "Nonfiction",
                 'publisher': "Les Solitaires Intempestifs", 'year': "2010",
                 'summary': "Prometheus defied the gods and gave fire to mankind", 'price': "24.34"}]
