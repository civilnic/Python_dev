...
#    print ("**EIS**")
    ...
    micdFile = FDEF_MICD("_tests_results/FDEF_EIS.xls", "fdef_EIS", 'V2.0')
    ...
    # deformatage
    #
    labelObjList_deformat = bdsEis.get_LabelObjList(nature="IN", system="EIS", source=r"FCDC.*|FCU.*|FG.*|^FM.*|FAC.*|SDAC.*")

    micdFile.deformat = True

    for labelObj in labelObjList_deformat:

        if len(labelObj.ParameterList) > 0:
             micdFile.AddLabelToMICD(labelObj)
             xml_conso_file.AddLabel(labelObj)
            # for parameterObj in labelObj.getParameterList():
             #    bds2xml_file.AddLine(parameterObj)
